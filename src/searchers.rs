...
impl<T, S> SearchIterator<(Option<T>, T), S, ParentMapQueue<(Option<T>, T), VecDeque<(Option<T>, T)>>>
where
    T: Clone + Eq + Debug + Hash,
    S: FnMut(&(Option<T>, T), &mut ParentMapQueue<(Option<T>, T), VecDeque<(Option<T>, T)>>),
{
    pub fn breadth_first(start: T, add_successors: S) -> Self {
        Self::new((None, start), ParentMapQueue::new(), add_successors)
    }

    pub fn bfs_tree(self) -> HashMap<T, Option<T>> {
        self.map(|(a, b)| (b, a)).collect()
    }
}

pub fn path_back<'a, T: Clone + Eq + Debug + Hash>(leaf: &T, bfs_tree: &'a HashMap<T, Option<T>>) -> impl Iterator<Item=T> + 'a {
    PathBackIter {bfs_tree: &bfs_tree, last: Some(leaf.clone())}
}

struct PathBackIter<'a, T: Clone + Eq + Debug + Hash> {
    bfs_tree: &'a HashMap<T, Option<T>>,
    last: Option<T>
}

impl<'a, T: Clone + Eq + Debug + Hash>  Iterator for PathBackIter<'a, T> {
    type Item = T;

    fn next(&mut self) -> Option<Self::Item> {
        let last = self.last.clone();
        self.last = self.last.as_ref().and_then(|last| self.bfs_tree.get(&last).unwrap().clone());
        last
    }
}
    ...
    S: FnMut(&T, &mut Q),
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.open_list.dequeue().map(|candidate| {
            (self.add_successors)(&candidate, &mut self.open_list);
...
mod tests {
    ...
    use crate::{multidim::Position, searchers::{path_back, SearchIterator, SearchQueue}};
    ...
    fn test_bfs() {
        ...
        let paths_back = SearchIterator::breadth_first(start_value, |(_, n), q| {
            if !visited.contains(&n) {
                visited.insert(n.clone());
                for neighbor in n.manhattan_neighbors().iter().filter(|neighbor| neighbor.manhattan_distance(&start_value) <= max_dist) {
                    q.enqueue(&(Some(*n), *neighbor));
                }
            }
        }).bfs_tree();
        ...
        for node in paths_back.keys() {
            let len = path_back(node, &paths_back).count();
